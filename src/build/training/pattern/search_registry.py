...
def resolve_strategy(s: Union[str, SearchStrategy]) -> SearchStrategy:
    if isinstance(s, str):
        try:
            ...
            raise KeyError(f"Unknown strategy '{s}'. Available: {list(STRATEGY_REGISTRY)}")
    return s
...
# --------- Built-in Strategies (ประกาศในไฟล์เดียวกันให้ครบจบ) ---------
@register("grid")
...
class RandomSearchStrategy(SearchStrategy):
    def _build(self, *, pipe, space, n_iter: int = 20, random_state: int = 42, **kwargs):
